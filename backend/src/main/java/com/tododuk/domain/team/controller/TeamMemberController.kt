...
class TeamMemberController(
    ...
    fun getTeamMembers(@PathVariable teamId: Int): RsData<List<TeamMemberResponseDto>> {
        ...
        val members = teamMemberService.getTeamMembers(teamId, authenticatedUser.id)
        return RsData.success("팀 멤버 목록 조회 성공", members)
    }
    ...
    fun createTeamMember(
        ...
        val authenticatedUser = getAuthenticatedUser()
        val teamMember = teamMemberService.addTeamMember(teamId, addDto, authenticatedUser.id)
        return RsData.success("팀 멤버가 성공적으로 추가되었습니다.", teamMember)
    }
    ...
    fun updateTeamMemberRole(
        ...
        val authenticatedUser = getAuthenticatedUser()
        val updatedMember = teamMemberService.updateTeamMemberRole(
            teamId,
            memberUserId,
            updateDto.role,
            authenticatedUser.id
        )
        return RsData.success("팀 멤버 역할이 성공적으로 변경되었습니다.", updatedMember)
    }
    ...
    fun deleteTeamMember(
        ...
        val authenticatedUser = getAuthenticatedUser()
        teamMemberService.deleteTeamMember(teamId, memberUserId, authenticatedUser.id)
